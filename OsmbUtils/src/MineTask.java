...
public class MineTask extends Task {
  ...
  private final boolean canMineMultipleMineables;
  private final List<String> mineableRockNames;
  private final Set<Integer> mineables;
  private final int respawnCircleZOffset;
  private final int respawnCircleDistanceTolerance;
  ...
  public MineTask(Script script,
                  ...
                  List<String> mineableRockNames,
                  Set<Integer> mineables,
                  Boolean canMineMultipleMineables,
                  Integer respawnCircleZOffset,
                  Integer respawnCircleDistanceTolerance) {
    super(script);
    this.canLoop = true;

    this.canMineMultipleMineables = canMineMultipleMineables != null && canMineMultipleMineables;
    this.mineableRockNames = mineableRockNames;
    this.mineables = mineables;
    this.respawnCircleZOffset = respawnCircleZOffset == null ? 25 :  respawnCircleZOffset;
    this.respawnCircleDistanceTolerance = respawnCircleDistanceTolerance == null ? 7 : respawnCircleDistanceTolerance;
  ...
  public MineTask(Script script,
                  ...
                  List<String> mineableRockNames,
                  Set<Integer> mineables) {
    this(script, scriptCore, mineableRockNames, mineables, null, null, null);
  }
  ...
  private boolean executeInternal() {
    ...
    boolean didMiningComplete = waitHelper.waitForNoChange(
      ...
      () -> {
        ...
        if (!canMineMultipleMineables && inventoryHelper.getSnapshot().getAmount(mineables) > initialMineablesCount) {
          script.log(getClass(), "Completed mining by way of received mineable");
          return true;
        }
  ...
  private List<RSObject> getVisibleSpawnedRocks(List<String> rockNames) {
    ...
    return entityHelper.filterUnspawnedObjects(
      visibleRocks, PixelAnalyzer.RespawnCircleDrawType.CENTER, respawnCircleZOffset, respawnCircleDistanceTolerance);
  }
  ...
  private List<RSObject> getMineableRocks(List<String> rockNames) {
    ...
    List<RSObject> vacantRocks = entityHelper.filterOccupiedObjects(scriptCore, visibleSpawnedRocks, true);
